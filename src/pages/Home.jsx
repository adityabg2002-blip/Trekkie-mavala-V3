...
export default function Home({ isMr, treks, reviews, settings, setActiveTab, onBook, heroType, heroUrl, onEditEntity, onEditText, refresh }) {
  ...
  const pinReview = async (r) => {
    await fetch('/api/data?resource=reviews', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: r.id, pinned: !r.pinned }) });
    refresh && refresh();
  };
  const delReview = async (id) => {
    if (!confirm('Delete this review?')) return;
    await fetch('/api/data?resource=reviews', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    refresh && refresh();
  };
