...
export default function Portal({ isMr, user, hikers, bookings, treks, tours, refresh }) {
  ...
  const cancel = async (id) => {
    await fetch('/api/data?resource=bookings', { method: 'PUT', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ id, status: 'Cancelled' }) });
    refresh();
  };
...
function ReviewForm({ isMr, hiker, treks = [], tours = [], refresh }) {
  ...
  const submit = async (e) => {
    ...
    setErr(''); setBusy(true);
    try {
      const res = await fetch('/api/data?resource=reviews', {
