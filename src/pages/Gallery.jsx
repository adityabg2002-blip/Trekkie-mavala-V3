...
export default function Gallery({ isMr, items, onEditText, refresh }) {
  ...
  const del = async (id) => {
    ...
    await fetch('/api/data?resource=gallery', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) });
    refresh();
  };
  ...
  const bulkDelete = async () => {
    ...
    setBusy(true);
    await fetch('/api/data?resource=gallery', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ids: selected }) });
  ...
  const handleUploaded = async (item) => {
    await fetch('/api/data?resource=gallery', {
