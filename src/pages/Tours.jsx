              ...
              <div key={t.id} style={{ '--i': idx % 3 }} className="reveal group relative bg-white border border-charcoal/10 overflow-hidden card-lift">
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button onClick={() => onBook({ id: t.id, kind: 'tour', name_en: t.title_en, name_mr: t.title_mr, price: t.price, date: t.date || '' })} className={`btn-sheen px-4 py-2 bg-charcoal text-cream text-xs font-bold uppercase tracking-wide hover:bg-heritage transition-colors ${isMr ? 'font-marathi' : ''}`}>{tr('bookNow', isMr)}</button>
                  </div>
                </div>
              </div>
            );
