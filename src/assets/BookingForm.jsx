...
export default function BookingForm({ trek, isMr, onClose, onDone }) {
  ...
  const payAndBook = async () => {
    ...
    try {
      ...
      // SECURITY: we send the item's id (not the price). The server looks up the
      // authoritative price from the database — the browser cannot dictate the
      // amount charged.
      const idPayload = trek?.kind === 'tour' ? { tourId: trek?.id } : { trekId: trek?.id };
      const orderRes = await fetch('/api/payment?action=order', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...idPayload, notes }),
      });
          ...
          try {
            const verifyRes = await fetch('/api/payment?action=verify', {
              ...
              body: JSON.stringify({
                ...
                booking: { ...f, age: f.age, trek_name: trekName, server_amount: order.serverAmount, ...idPayload },
                lang: isMr ? 'mr' : 'en',
              }),
            });
