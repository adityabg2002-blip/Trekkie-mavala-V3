  ...
  const loadAll = async () => {
    try {
      const [t, to, g, r, f, b, h, s, c] = await Promise.all([
        fetch('/api/data?resource=treks').then(x => x.json()),
        fetch('/api/data?resource=tours').then(x => x.json()),
        fetch('/api/data?resource=gallery').then(x => x.json()),
        fetch('/api/data?resource=reviews').then(x => x.json()),
        fetch('/api/data?resource=faqs').then(x => x.json()),
        fetch('/api/data?resource=bookings').then(x => x.json()),
        fetch('/api/data?resource=hikers').then(x => x.json()),
        fetch('/api/data?resource=business-settings').then(x => x.json()),
        fetch('/api/data?resource=site-content').then(x => x.json()),
      ]);
