...
export default function Header({ activeTab, setActiveTab, isMr, setIsMr, content, onEditText }) {
  ...
  const saveLogo = async (url) => {
    await fetch('/api/data?resource=site-content', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content_key: 'logo_url', value_en: url, value_mr: url, content_type: 'media' }) });
