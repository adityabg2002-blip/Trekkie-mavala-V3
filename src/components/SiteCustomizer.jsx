...
export default function SiteCustomizer({ content, settings, refresh }) {
  ...
  const saveHero = async (type, url) => {
    await fetch('/api/site-content', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content_key: 'hero_media_type', value_en: type, value_mr: type, content_type: 'config' }) });
    await fetch('/api/site-content', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content_key: 'hero_media_url', value_en: url, value_mr: url, content_type: 'media' }) });
    await refresh();
  };
  ...
  const saveBiz = async () => {
    setSaving('biz');
    try {
      await fetch('/api/business-settings', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(b) });
      await refresh();
      setBiz(null);
    } finally { setSaving(''); }
  };
