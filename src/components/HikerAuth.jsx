...
export default function HikerAuth({ isMr, onAfterAuth }) {
  ...
  const emailSubmit = async (e) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        await fetch('/api/hikers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, mobile, email, auth_email: email }) });
  ...
  const mobilePassSubmit = async (e) => {
    ...
    try {
      ...
      if (mode === 'signup') {
        ...
        const res = await fetch('/api/otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'register', mobile: digits, password: pass, name, email: email || null }) });
        ...
        const r = await fetch('/api/otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'resolve', mobile: digits }) });
  ...
  const sendOtp = async (e) => {
    ...
    try {
      ...
      if (digits.length < 10) throw new Error(isMr ? 'वैध मोबाईल नंबर टाका.' : 'Enter a valid mobile number.');
      const res = await fetch('/api/otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'send', mobile: digits }) });
      const data = await res.json();
  ...
  const verifyOtp = async (e) => {
    ...
    try {
      const digits = mobile.replace(/\D/g, '');
      const res = await fetch('/api/otp', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action: 'verify', mobile: digits, code: otp, name: name || null, email: email || null }) });
      const data = await res.json();
