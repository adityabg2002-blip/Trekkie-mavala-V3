...
export default function Commander({ isMr, treks, tours, bookings, gallery, hikers, reviews, content, settings, refresh, onEditEntity }) {
  ...
  useEffect(() => {
    if (!isAdmin) return;
    fetch('/api/trek-bookings').then((r) => r.json()).then((d) => setPayStats(d.stats || null)).catch(() => {});
  }, [isAdmin]);
  ...
  const login = async (e) => {
    ...
    setErr(''); setBusy(true);
    try {
      const res = await fetch('/api/admin-config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ admin_id: id, passcode: pass }) });
  ...
  const delTrek = async (id) => { if (!confirm('Delete this trek?')) return; await fetch('/api/treks', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); refresh(); };
  const delTour = async (id) => { if (!confirm('Delete this tour?')) return; await fetch('/api/tours', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); refresh(); };
  const delGallery = async (id) => { await fetch('/api/gallery', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); refresh(); };
  const pinReview = async (r) => { await fetch('/api/reviews', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: r.id, pinned: !r.pinned }) }); refresh(); };
  const delReview = async (id) => { if (!confirm('Delete this review?')) return; await fetch('/api/reviews', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) }); refresh(); };
...
function SecurityPanel({ isMr, adminPass }) {
  ...
  const change = async (e) => {
    ...
    setMsg(''); setBusy(true);
    try {
      const res = await fetch('/api/admin-config', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ current_passcode: cur, admin_id: newId || undefined, passcode: newPass || undefined }) });
      const data = await res.json();
      if (res.ok && data.ok) {
