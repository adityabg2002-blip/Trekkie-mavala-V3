...
// Maps a mobile number to a deterministic internal email + password for Supabase auth.
// SMS gateway is not provisioned, so OTPs are generated server-side and returned to the
// client in "demo delivery" mode (also stored so verify can check them). In production you
// would send `code` via Twilio/MSG91 instead of returning it.
...
function mobileToPassword(mobile) {
  const digits = String(mobile).replace(/\D/g, '');
  return `MavalaOtp#${digits}`; // stable internal password for OTP-based accounts
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'send') {
      ...
      // clear old codes for this mobile, then insert fresh
      ...
      // In production, send SMS here. For this app we return the code so the UI can show it.
      return res.status(200).json({ ok: true, demoCode: code });
    }

    // ---- VERIFY OTP -> ensure account exists -> return credentials for client sign-in ----
    if (action === 'verify') {
      ...
      // Find existing auth user by internal email
      let userId = null;
    ...
    // ---- resolve mobile -> internal email (for password sign-in) ----
    if (action === 'resolve') {
      const { mobile } = req.body;
      const digits = String(mobile || '').replace(/\D/g, '');
...
async function ensureHiker(mobile, name, email) {
  try {
    ...
    await supabase.from('hiker_profiles').insert({
      name: name || `Hiker ${String(mobile).slice(-4)}`,
      mobile,
      email: email || null,
