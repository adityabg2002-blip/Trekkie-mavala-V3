// Consolidated data API — handles all simple DB resources through one serverless
// function to stay within Vercel Hobby's 12-function limit.
// Usage: /api/data?resource=treks  (GET/POST/PUT/DELETE)
...
export default async function handler(req, res) {
  ...
  try {
    switch (resource) {
      ...
      case 'admin-config': return adminConfigHandler(req, res);
...
async function reviewsHandler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { data, error } = await supabase.from('hiker_reviews').insert({ name, hiker_email, rating: rating || 5, text_en, text_mr: text_mr || text_en, trek, pinned: false, approved: true }).select().single();
    if (error) throw error;
    return res.status(201).json(data);
  }
  if (req.method === 'PUT') {
...
// ---------- BOOKINGS (legacy trek bookings list) ----------
