...
const BASE = ['title_en','title_mr','region','days','price','description_en','description_mr','image'];
const EXTRA = ['route_en','route_mr','venue_en','venue_mr','pickup_en','pickup_mr','date','departure_time','show_date'];
...
async function attachExtras(rows) {
  ...
  const { data } = await supabase.from('entity_extras').select('*').eq('entity_type', 'tour').in('entity_id', ids);
  const map = {};
...
async function saveExtras(id, extra) {
  ...
  const { data: existing } = await supabase.from('entity_extras').select('id,data').eq('entity_type', 'tour').eq('entity_id', id).maybeSingle();
  if (existing) {
    await supabase.from('entity_extras').update({ data: { ...(existing.data||{}), ...extra }, updated_at: new Date().toISOString() }).eq('id', existing.id);
  } else {
    await supabase.from('entity_extras').insert({ entity_type: 'tour', entity_id: id, data: extra });
  }
}
